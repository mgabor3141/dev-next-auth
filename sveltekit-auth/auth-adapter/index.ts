...
const toAdaptAuthenticator: (a?: TAuthenticator) => AdapterAuthenticator | null = (a) => {
	if (a) {
		return {
			counter: a.counter,
...
const toVerificationToken: (c?: VerificationToken) => VerificationTokenT | null = (c) => {
	if (c) {
		return {
			expires: c.expires,
...
export const RemultAdapter: (args: {
	...
	const Authenticator = args?.customEntities?.Authenticator ?? local_Authenticator;
		...
		adapter: new Proxy(
			{
				async createVerificationToken(verificationToken) {
					return toVerificationToken(await repo(VerificationToken).insert(verificationToken));
				},
				...
				async getSessionAndUser(sessionToken: string | undefined): Promise<{
					...
					const session = toAdapaterSession(await repo(Session).findFirst({ sessionToken }));
					if (!session) {
						return null;
					}

					const user = toAdapterUser(await repo(User).findFirst({ id: session.userId }));
					if (!user) {
						return null;
					}
				...
				async createAuthenticator(authenticator) {
					return toAdaptAuthenticator(await repo(Authenticator).insert(authenticator)!)!;
				},
				async getAccount(providerAccountId, provider) {
					return toAdapterAccount(await repo(Account).findFirst({ providerAccountId, provider }));
				},
				async getAuthenticator(credentialID) {
					return toAdaptAuthenticator(await repo(Authenticator).findFirst({ credentialID }));
				},
